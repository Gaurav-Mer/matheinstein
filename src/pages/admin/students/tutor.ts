...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        let query: any = adminDb.collection("users").where("students", "array-contains", studentId);

        const offset = (pageNum - 1) * limitNum;
        query = query.limit(limitNum).offset(offset);

        const tutorsSnapshot = await query.get();
        const tutors = tutorsSnapshot.docs.map((doc: any) => ({ uid: doc.id, ...doc.data() }));
